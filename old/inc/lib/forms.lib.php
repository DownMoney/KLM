 ...
 * forms.php
 *
 * @(#) $Header: /home/mlemos/cvsroot/forms/forms.php,v 1.167 2004/09/08 05:02:44 mlemos Exp $
 *
 */
...
class form_class
{
	...
	Function AddInput($arguments)
	{
		...
		switch(($input["TYPE"]=$arguments["TYPE"]))
		{
			...
			case "select":
				...
				if( wt_is_valid( $arguments["OPTIONS_DISABLED"], 'array' ) ) {
					$input['OPTIONS_DISABLED'] = $arguments["OPTIONS_DISABLED"];
				}
		...
		if(IsSet($arguments["LABEL_CLASS"]))
		{
			$input["LABEL_CLASS"]=$arguments["LABEL_CLASS"];
		}
	...
	Function AddInputPart($input, $params = array())
	{
		if(wt_is_valid($params, 'array') && wt_is_valid($this->inputs[$input], 'array')) {
			$this->inputs[$input] = array_merge($this->inputs[$input], $params);
		}
	...
	Function AddLabelPart($arguments, $params = array())
	{
		...
		if(IsSet($arguments["LABEL_CLASS"]))
			$label["LABEL_CLASS"]=$arguments["LABEL_CLASS"];
		...
		if(wt_is_valid($params, 'array') && wt_is_valid($label, 'array')) {
		  	$label = array_merge($label, $params);
		}
	...
	Function OutputOnChangeAttribute(&$input,$function)
	{
		...
		&& count($input["ReplacePatterns"]))
		{
			...
			$onchange.="if(new_value.replace) { new_value=".HtmlSpecialChars($value)."; } ; ";
		}
		if(strlen($onchange))
	...
	Function OutputInput(&$input, $input_id, $input_read_only, $function, $eol, &$resubmit_condition)
	{
		switch($input["TYPE"])
		{
		  ...
		  if(is_array($input["OPTIONS"]) && count($input["OPTIONS"]) > 0) {
				...
				$_options_count = count($options)-1;
				$label_open = false;
				...
				for($space="",$option=0,Reset($options);$option<count($options);Next($options),$option++)
				{
					...
					if(!$input_read_only)
						...
						if(substr(Key($options), 0, 8) == '__LABEL_') {
							$is_label = true;
						}
					...
					if($is_label === true) {
							if($label_open === true) {
							$function("</optgroup>$eol");
							}
							$function("<optgroup label=\"".$this->EncodeHTMLString($options[Key($options)])."\">$eol");
							$label_open = true;
				  ...
				  if( IsSet($input["OPTIONS_DISABLED"]) && array_keys($input["OPTIONS_DISABLED"], Key($options)) ) {
							$function(" disabled=\"disabled\" readonly=\"readonly\" ");
				  }
					...
					if(!$input_read_only) {
						...
						if($label_open === true && $_options_count == $option) {
							$function("</optgroup>$eol");
							}
						}
						}
				...
				else
				{
					...
					switch($input["TYPE"])
					{
						...
						case "text":
							...
							&& IsSet($input["VALUE"]))
								$function(" value=\"".htmlspecialchars($input["VALUE"])."\"");
						case "file":
	...
	Function Output($arguments)
	{
		...
		|| $this->accessible_parts)
		{
			...
			if(strcmp($this->ID,"")) {
				$function(" id=\"$this->ID\"");
				$this->ValidationFunctionName = $this->ValidationFunctionName . $this->ID;
				}
			...
			&& $this->client_validate)
			{
				for($script_files=$password_fields=array(),$input_part=0;$input_part<count($this->input_parts);$input_part++)
				{
					...
					|| !strcmp($part_type,"ACCESSIBLE_INPUT")))
					{
						...
						{
							$function("<script type=\"text/javascript\" src=\"".$input["EncodingFunctionScriptFile"]."\">\n</script>\n");
							$script_files[$input["EncodingFunctionScriptFile"]]=$this->input_parts[$input_part];
						}
					}
				}
			}
			$function("<script type=\"text/javascript\">$eol $jseol");
	...
	if(strcmp($resubmit_condition,""))
			...
			&& $this->client_validate)
			{
				...
				$function("$this->ValidationFunctionName = function(theform)$eol{".$eol);
				...
				case "READ_ONLY_LABEL":
				  ...
				  if(($this->ReadOnly
					...
					|| $this->types[$this->inputs[$label["FOR"]]["Part"]]!="ACCESSIBLE_INPUT"))
						...
						$function("<label for=\"".$label["FOR"]."\" id=\"".$label["FOR"]."_label\" ".(IsSet($label["ACCESSKEY"]) ? " accesskey=\"".$label["ACCESSKEY"]."\"" : "")."".(IsSet($label["LABEL_CLASS"]) ? " class=\"".$label["LABEL_CLASS"]."\"" : "").">".$label["LABEL"]."</label>");
					break;
			}
		}
		if(!$this->ReadOnly
